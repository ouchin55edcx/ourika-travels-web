  ...
  return (
    ...
    >
      <div className="relative mb-2 overflow-hidden rounded-[12px]">
        ...
        {trek.badge && (
          <div className="absolute top-2 left-2 rounded-[8px] bg-[#004f32] px-2 py-1 text-[10px] leading-none font-extrabold text-white">
            {trek.badge}
          </div>
        )}
      </div>
      ...
      <div className="space-y-1.5">
        ...
        <h3 className="line-clamp-3 text-[15px] leading-6 font-extrabold text-[#0a2e1a]">
          {trek.title}
        </h3>

        <div className="flex flex-wrap items-center gap-1.5 text-[12px]">
          <span className="font-bold text-[#0a2e1a]">{trek.rating.toFixed(1)}</span>
          <div className="flex items-center gap-1">
            {[0, 1, 2, 3, 4].map((i) => (
              <span
                key={i}
                className={`h-2.5 w-2.5 rounded-full ${
                  i < Math.round(trek.rating) ? "bg-[#00aa6c]" : "border border-[#00aa6c] bg-white"
                }`}
              />
            ))}
          </div>
        ...
        <div className="flex items-end justify-between gap-3 pt-1">
          <p className="text-[15px] font-extrabold text-[#0a2e1a]">
            from{" "}
            {trek.previous_price && (
