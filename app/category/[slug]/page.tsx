...
import { experiencesData, ExperienceItem } from "@/app/experiences/components/experiencesData";
import {
    ChevronLeft,
    Compass,
    Utensils,
...
const categories = {
    outdoors: {
        ...
        check: (item: ExperienceItem) =>
            ["Adventure Tours", "Day Trips", "4WD Tours", "Air Tours", "Nature Walks", "Camel Rides"].includes(item.category) ||
...
export default function CategoryPage() {
    ...
    const filteredItems = useMemo(() => {
        ...
        return experiencesData.filter(item => {
            ...
            // Language filter
            if (activeFilters.language !== "All" && !item.languages.includes(activeFilters.language)) return false;

            // Time of day filter
            if (activeFilters.timeOfDay !== "All" && item.timeOfDay !== activeFilters.timeOfDay) return false;

            return true;
        });
    }, [category, activeFilters]);

    if (!category) return null; // Add basic redirect or error handling here
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div>
                            ...
                            <p className="text-base text-gray-500 font-medium">
                                Showing {filteredItems.length} {filteredItems.length === 1 ? 'experience' : 'experiences'} based on your interests
                            </p>
                        </div>
                    </div>
                </div>
                        ...
                        {(Object.keys(filterOptions) as Array<keyof CategoryFilters>).map((key) => (
                            <div key={key} className="relative shrink-0">
                                <button
                                    ...
                                    className={`flex items-center gap-2 px-4 py-2 border rounded-full text-sm font-bold transition-all ${activeFilters[key] !== "All"
                                            ? "border-[#004f32] bg-[#f0f9f6] text-[#004f32]"
                                            : "border-gray-200 bg-white hover:border-gray-400 text-gray-700"
                                        }`}
                                >
                                    {key.charAt(0).toUpperCase() + key.slice(1)}
                                    {activeFilters[key] !== "All" && <span className="text-xs">({activeFilters[key]})</span>}
                                        ...
                                        <div className="absolute top-full left-0 mt-2 w-56 bg-white border border-gray-100 rounded-2xl shadow-2xl p-2 z-10 animate-in fade-in zoom-in-95 duration-200">
                                            {filterOptions[key].map((option) => (
                                                <button
                                                    ...
                                                    className={`w-full text-left px-4 py-2.5 rounded-xl text-sm font-bold transition-colors ${activeFilters[key] === option
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
                ...
                >
                    {filteredItems.length > 0 ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-20 text-center">
                            ...
                            <h2 className="text-2xl font-black text-gray-900 mb-2">No matching results</h2>
                ...
                <div className="mt-20 p-8 md:p-12 bg-[#f0f9f6] rounded-[2.5rem] border border-[#d1ede1]">
                    <div className="flex flex-col md:flex-row gap-8 items-start">
                        ...
                        <div>
                            <h3 className="text-2xl font-black text-[#004f32] mb-3">About {category.title} in Ourika</h3>
                            <p className="text-gray-700 font-medium leading-[1.7] max-w-4xl">
                                {category.subtitle} Whether you're looking for an afternoon getaway or a full-day immersive journey, Ourika Travels offers the most authentic local perspectives. Our guides are experts in the region's history, culture, and hidden gems.
                            </p>
                        </div>
                    </div>
                </div>
