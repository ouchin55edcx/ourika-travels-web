...
import { getCachedActiveGuides } from "@/lib/admin-cache";
...
export async function getAllBookings(offset = 0, limit = 50): Promise<any[]> {
  const supabase = await createSupabaseServerClient();
...
export async function createWalkinBooking(data: {
  ...
  const pricePerChild = pricePerAdult * 0.5;
  const totalPrice = data.adults * pricePerAdult + (data.children ?? 0) * pricePerChild;

  const { data: refData } = await supabase.rpc("generate_booking_ref");
  const booking_ref = refData ?? "OT-WI-" + Date.now().toString(36).toUpperCase();
