...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `${BASE_URL}/guide/${getGuideSlug(user)}`,
      images: user.avatar_url ? [{ url: user.avatar_url, width: 400, height: 400, alt: name }] : [],
    },
    alternates: { canonical: `${BASE_URL}/guide/${getGuideSlug(user)}` },
  };
}
