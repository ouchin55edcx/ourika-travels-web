...
import { wishlistData } from "@/lib/data/wishlist";
      ...
      <div className="relative z-10 flex flex-col gap-5">
        {items.length === 0 ? (
          <div className="rounded-[24px] border border-dashed border-[#d6d7cc] bg-white/70 p-10 text-center">
            <p className="text-[20px] font-black text-[#1f2937]">Your wishlist is empty</p>
          ...
          items.map((experience) => (
            <WishlistCard key={experience.id} experience={experience} onRemove={handleRemove} />
          ))
        )}
      </div>
