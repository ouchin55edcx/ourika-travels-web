        ...
        <ul className="grid gap-3">
          {highlights.filter(Boolean).map((h, i) => (
            <li key={i} className="flex items-start gap-3 py-1 text-[15px] text-[#1f1f1f]">
              <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-[#1E9E6A]" />
              {h}
            </li>
          ))}
        </ul>
...
SERVICIOS
        ...
        <div className="flex flex-wrap gap-2">
          {services.filter(Boolean).map((s, i) => (
            <span
              key={i}
              className="inline-flex items-center gap-1.5 rounded-full border border-[#d0ede0] bg-[#edf7f1] px-3 py-1.5 text-[13px] font-semibold text-[#12355B]"
            >
