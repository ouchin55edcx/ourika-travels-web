...
export default function Interests({ initialCategories = [] }: InterestsProps) {
  ...
  return (
    <section id="interests-section" ref={elementRef as any} className={`mx-auto mt-10 w-full max-w-7xl bg-[#FAFAF7] px-6 md:mt-12`}>
      ...
      <div className="hide-scrollbar flex snap-x snap-mandatory gap-4 overflow-x-auto pb-5">
        {displayedInterests.map((interest, index) => {
          const isFavorite = favorites[interest.slug];
          return <Link key={interest.title} href={`/category/${interest.slug}`} className={`group reveal relative h-[330px] min-w-[78%] flex-shrink-0 cursor-pointer snap-center overflow-hidden rounded-3xl border border-transparent shadow-lg transition-all duration-500 hover:border-white hover:shadow-xl sm:min-w-[54%] md:min-w-[42%] lg:h-[390px] lg:min-w-[calc(25%-12px)] ${isVisible ? "reveal-visible" : ""}`} style={{ transitionDelay: `${index * 120}ms` }}>
