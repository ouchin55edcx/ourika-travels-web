...
export function FAQ() { const [open, setOpen] = useState(0); return <section id="faq-section" className="bg-[#FAFAF7] px-6 py-20 md:py-28"><div className="mx-auto max-w-3xl"><h2 className="mb-10 text-center text-4xl font-black text-[#12355B]">Preguntas frecuentes</h2><div className="divide-y divide-[#12355B]/10 rounded-2xl border border-[#12355B]/10 bg-white px-6">{faqs.map(([question, answer], index) => <div key={question}><button onClick={() => setOpen(open === index ? -1 : index)} className="flex w-full items-center justify-between py-5 text-left font-black text-[#12355B]"><span>{question}</span><ChevronDown className={`h-5 w-5 transition-transform ${open === index ? "rotate-180" : ""}`} /></button>{open === index && <p className="pb-5 pr-8 text-sm leading-6 text-[#6B7280]">{answer}</p>}</div>)}</div></div></section>; }
